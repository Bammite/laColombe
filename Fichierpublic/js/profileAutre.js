...
async function loadUserData() {
    try {
        const response = await fetch(`/api/user/${getUserIdFromURL()}`); 
        const données = await response.json();
        console.log(données.data);
        if (response.ok) {
            userid=données.data.user._id;
            updateUserInterface(données.data);
...
function updateUserInterface(userData) {
    ...
    document.getElementById('prenom').textContent = userData.personnel.prenom;
    document.getElementById('nom').textContent = userData.personnel.nom;
    document.getElementById('role').textContent = userData.personnel.role;
    
    // Statut avec classe CSS
    const statutElement = document.getElementById('statut');
    statutElement.textContent = userData.personnel.statut || 'Iconnu';
    statutElement.className = `statut ${userData.personnel.statut.toLowerCase()}`;

    // Photo de profil
    if (userData.personnel.photo) {
        document.getElementById('profile-pic').src = getImageUrl(userData.personnel.photo);
    }

    // Informations personnelles
    document.getElementById('email').textContent = userData.personnel.email;
    document.getElementById('telephone').textContent = userData.personnel.telephone;
    document.getElementById('adresse').textContent = userData.personnel.adresse;
    document.getElementById('dateNaissance').textContent = formatDate(userData.personnel.dateNaissance);
    document.getElementById('numeroCarteIdentite').textContent = userData.personnel.numeroCarteIdentite || 'Non renseigné';
    ...
    if (userData.role === 'Employé') {
        ...
        document.getElementById('poste').textContent = userData.personnel.poste;
        document.getElementById('departement').textContent = userData.personnel.departement;
        document.getElementById('typeContrat').textContent = userData.personnel.typeContrat;
        document.getElementById('dateEmbauche').textContent = formatDate(userData.personnel.dateEmbauche);
        document.getElementById('salaire').textContent = formatSalaire(userData.personnel.salaire);
    } else {
        employeeSection.style.display = 'none';
    }

    // Historique
    document.getElementById('dernierAcces').textContent = formatDate(userData.personnel.dernierAcces);
    document.getElementById('dateCreation').textContent = formatDate(userData.personnel.dateCreation);
...
document.getElementById('password-form').addEventListener('submit', async function(e) {
    ...
    try {
        const response = await fetch(`/changer-mot-de-passe/${userid}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
